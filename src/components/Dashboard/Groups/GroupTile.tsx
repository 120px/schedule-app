...
const GroupTile: React.FC<GroupTileProps> = ({ group }: GroupTileProps) => {
    ...
    const handleGroupClick = () => {
        setCurrentGroup({id: group.id, name: group.groupName});
    }
    ...
    return (
        <div className="flex flex-col rounded-xl overflow-hidden shadow-sm bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow group">
            <div className='h-48 w-full bg-center bg-no-repeat bg-cover relative' style={{backgroundImage: `url('https://source.unsplash.com/random/400x300/?group,team,${group.id}')`}}>
            ...
            <div className="p-5 flex flex-col gap-3">
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="text-[#181310] dark:text-white text-lg font-bold leading-tight group-hover:text-primary transition-colors">{group.groupName}</h3>
                        <p className="text-[#8d6d5e] text-sm font-normal mt-1 line-clamp-2">{group.description || "No description provided."}</p>
                    </div>
                </div>
                <div className="flex items-center justify-between mt-2 pt-4 border-t border-gray-50 dark:border-gray-700">
                    <div className="flex items-center gap-2">
                        <span className="text-[#8d6d5e] text-xs font-medium">{group.members ? group.members.length : 0} Members</span>
                    </div>
                    <Link 
